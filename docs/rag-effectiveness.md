...
- **Baseline Run**: VLM evaluation run with `retrievedExamples` forcibly set to `[]` (empty array).
- **RAG-Enabled Run**: VLM evaluation run with top-scoring precedent retrieved from the Vault via `scoreSimilarity()`.
...
| **Ambiguous Selector Resolution** | Re-queried DOM 1x on missing ID | Resolved correct button on 1st pass | **Reduced iteration retries** |
