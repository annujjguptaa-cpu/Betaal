...
// =========================================================================
// POPUP INITIALIZATION & REACTIVE STATE REHYDRATION (Module 71)
// =========================================================================
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Model Cache Clear Button (Module 72)
  ...
  // Fetch initial background loop state upon opening popup (Module 71)
...
// =========================================================================
// REACTIVE LISTENER FOR BACKGROUND LOOP EVENTS (Modules 70, 71, 78, 79, 80)
// =========================================================================
...
browser.runtime.onMessage.addListener(async (message) => {
  ...
  // Module 78 & 79: Live feed item update — render single card as it arrives
...
async function syncWithBackgroundLoopState() {
  try {
    ...
    if (res && res.success && res.state) {
      ...
      // Module 78: Replay full feed history when popup is (re)opened mid-task
...
function applyLoopStateToUI(state) {
  ...
  // Module 79: Update feed progress bar
  ...
  // Sync DOM stability & pacing timing display if present (Modules 68 & 76)
...
// =========================================================================
// RUN AGENT TRIGGER (Modules 68-71)
// =========================================================================
...
async function triggerAgentLoop(resumeAction = null) {
  ...
  try {
    ...
    // Send START_LOOP to background script (Module 70 & 71)
...
// =========================================================================
// ACTIVITY FEED RENDERER (Modules 78, 79, 80)
// =========================================================================
...
function renderActivityFeed(feedItems) {
  ...
  // Wire inline approve/stop buttons (Module 80)
  ...
  // Module 81: Wire correction input submit (button click + Enter key)
...
function renderPipelineArtifacts(pipelineRes) {
  ...
  // Render timing breakdown telemetry (Module 68 + 72)
  if (liveTimerEl) {
    liveTimerEl.style.display = 'block';
  ...
  // Render Degradation Notes banner if any detector failed gracefully (Module 73)
...
// =========================================================================
// RUN MODE COMPARISON (Module 72 session timing comparison)
// =========================================================================
...
async function runModeComparison() {
  ...
  try {
    ...
    // Run Balanced Mode (benefits from session caching across all model types - Module 72)
...
async function renderVaultTab() {
  ...
  vaultListContainer.innerHTML = entries.map((entry) => {
    ...
    // Module 81: Distinct badge for user-corrected steps
    ...
    // Module 81: Show correction text if present
...
// =========================================================================
// NOTIFICATIONS TAB RENDERING (Module 71 - Background Linked)
// =========================================================================
...
// Global Intervention Decision Functions (Modules 71 & 80)
window.approveIntervention = async function (id) {
  ...
  if (item) {
    ...
    // Module 80: Immediately update the feed card in-place (before background responds)
    ...
    // Module 80: Keep notifications tab in sync
    renderNotificationsTab();
...
window.stopIntervention = async function (id) {
  ...
  if (item) {
    ...
    // Module 80: Immediately update the feed card in-place
    const feedContainer = document.getElementById('activity-feed-container');
    if (feedContainer) {
      const card = feedContainer.querySelector(`[data-step-id*="${id}"], .feed-card.paused`);
      if (card) {
...
// =========================================================================
// PROFILE TAB (Module 82) — Local Identity Store, Never Leaves the Device
// =========================================================================
...
async function renderPolicyTab() {
  ...
  // Module 90: Bind Backend URL Override Settings
